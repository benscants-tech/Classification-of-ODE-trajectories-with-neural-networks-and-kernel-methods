...
beta = np.zeros(n_training_examples)
...
def update_beta(indices, beta, learning_rate, y):
    beta[indices] += learning_rate * (y[indices] - sigmoid((kernel @ beta)[indices]))
for i in range(1000):
    indices = np.random.choice(np.arange(n_training_examples), size=n_training_examples, replace=False)
